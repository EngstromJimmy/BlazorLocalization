...
In this sample, I will show you how to localize your content using the NuGet package **Toolbelt.Blazor.I18nText**.
The built-in support for localization is using resx files which requires a specific editor to work.
I think it is a bit nicer to be able to edit the files using JSON instead.
...
This sample is using cookies but could just as easily used localstorage intested.
...
In the Pages folder add a new file called **CultureSelector.razor**
...
2. Add a the new component to the layout
...
1. In Program.cs add ```builder.Services.AddI18nText();```
